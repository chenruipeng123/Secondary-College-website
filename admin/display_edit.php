<?php 
	...
	$sql = "select * from display where id='{$_GET['mid']}'";
	...
	if (!empty($_POST['show_order'])&&!empty($_POST['title'])) {
		if (isset($_GET['a'])&&$_GET['a']=='edit') {
			...
			if (!empty($oldfilename)) {
				...
				// var_dump($ins_menu);
				// exit;
				$mes = $mysqlObj->update("display",$ins_menu,$cond);
				if ($mes) {
					alertMes("修改成功","display.php");
				}else{
					alertMes("修改失败","display_edit.php?mid={$_GET['mid']}");
					exit;
				}
			}else{
				$ins_menu = $_POST;
				...
				<div class="col-sm-12">
					<div class="tabbable">
						...
						<div class="tab-content">
							<div class="row">
								<div class="col-xs-12">
									<form action="display_edit.php?mid=<?php echo $_GET['mid'] ?>&a=edit" class="form-horizontal" 
