				...
				<div class="col-xs-12">
					...
					if (isset($_GET['a']) && $_GET['a'] == 'add') {
						...
						$mes = $mysqlObj->insert2("menu_cont", $ins);
								...
								// exit;
						if ($mes) {
							alertMes("添加成功！", "pic.php?id={$_GET['id']}&up_id={$_GET['up_id']}");
						} else {
							alertMes("添加失败！", "pic_add.php?id={$_GET['id']}&up_id={$_GET['up_id']}");
							exit;
						}
					}
					?>
						<div class="tab-content">
							<form action="pic_add.php?id=<?php echo $_GET['id'] ?>&up_id=<?php echo $_GET['up_id'] ?>&a=add" class="form-horizontal" role="form" method="post" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<div class="col-sm-10">
										<input type="text" name="addr" placeholder="请输入链接地址(可不填)" class="col-xs-10 col-sm-5">
									</div>
